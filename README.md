...
- **微调模型管理(专属音色包)**:上传「GPT+SoVITS 权重 + 参考音频」三件套注册专属音色,
  一键启用/切回底模(热切换, 重启自动恢复), **双模式自动路由**(克隆↔专属, 设备无感)
- **管理后台**:流式试音(含进阶采样参数)、音色注册/编辑(改ID·转写·替换音频·绑定模型)/删除/试听、备份/恢复
- **11 种语言模式**:中(中英混)/日(日英混)/英/韩/粤/多语种自动/强制单语种
- **自愈运维**:一键启停、三语预热、10 分钟心跳 + 看门狗自动拉起、GPU keeper 防降频、日志轮转
- **一键自检**:`selftest.sh` 回归测试全部端点(13 项)
